...
export default function DataTableDemo() {
  ...
  useEffect(() => {
    const fetchTeachersData = async () => {
      const querySnapshot = await getDocs(collection(db, "teachers-atendance"));
  ...
  useEffect(() => {
    const fetchAttendanceData = async () => {
      const querySnapshot = await getDocs(collection(db, "teachers-atendance")); // Change to the correct collection for attendance
      const data = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      })) as Attendance[];
